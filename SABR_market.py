...
def launch_sabr(param): 
    ...
    if param_name in parameters: 
        param_pos = parameters.index(param_name)  
        starting_guess[param_pos] = param[1] 
    ...
    def newobjfunc(params,F,K,time,MKT): 
        par = [0]*4
        par[param_pos] = param[1] 
        i = 0 
        for j in range(4):
            if j==param_pos:
                i = j
        ...
        return objfunc(par,F,K,time,MKT)

    def calibration(starting_par,F,K,time,MKT):
        if param_pos is not None: 
            for i in range(len(F)):
                x0 = list(starting_par)
                bnds = [(0.001,None) , (0,1) , (-0.999,0.999) , (0.001,None) ]
                bnds.pop(param_pos)
                x0.pop(param_pos)
                x0 = numpy.array(x0) 
                res = minimize(newobjfunc, x0, (F[i],K[i],time[i],MKT[i]), bounds = bnds, method='SLSQP') 
                resx = list(res.x)
                resx.insert(param_pos, param[1])
                alpha[i] = resx[0]
                beta[i] = resx[1]
                rho[i] = resx[2]
                nu[i] = resx[3]
